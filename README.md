...
class Storage {
  setItem(key, val) {
    ...
    storage[key] = val
    window.localStorage.setItem(projectConfig.namespace, JSON.stringify(storage))
  }
  getItem(key) {
    return this.getStorage()[key] || null
  }
  getStorage() {
    const storage = JSON.parse(window.localStorage.getItem(projectConfig.namespace) || '{}')
    return storage
  }
  clearItem(key) {
